...
Upstream ref: ServerInventorySql / ServerMetricsSql / HardwareNoteFor
(ViewerDataService.FinOps.Inventory.cs), ServerPropertyRow's computed columns
(ViewerDataService.FinOps.cs), LoadFinOpsServerInventoryAsync.

Cross-server by design, so no $server variable: upstream's query carries no server filter.
"""
